...
class WeatherRepository @Inject constructor(
    ...
    private val cityInfoDao = WeatherDatabase.getDataBase(context = context).cityInfoDao()

    suspend fun getWeatherNew(location:String,lang:Lang)  =
        suspendCancellableCoroutine<WeatherNowBean.NowBaseBean> {
            continuation ->
            QWeather.getWeatherNow(context,location,lang,Unit.METRIC,object :QWeather.OnResultWeatherNowListener{
                ...
                override fun onSuccess(p0: WeatherNowBean) {
                    if (Code.OK === p0.code){
                        continuation.resume(p0.now)
                    }else{
                        val code:Code = p0.code
